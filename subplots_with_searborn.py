...
df = df[["prSM [db]", "İstasyon", "year", "ChlA","S(psu)","NO3+NO2(µM)","Si(µM)","PO4(µM)","new_year"]] #belli columnları alır
df = df.loc[( df["prSM [db]"] ==1)] # df=df [df["prSM [db]"] == 1] ile aynı şey. (db si 1 olan bütün sütunlar alır)
df = df.loc[df["İstasyon"].isin(["M8", "M14", "M23", "MY2"])] #hangi istansyonları ayıklıyoruz..
...
sns.lineplot(data=df, x="year", y="NO3+NO2(µM)", hue="İstasyon",lw=0.65,legend=False,ax=axes[0]) #subplots yapınca ax= diye hangisi oldugunu belirt
sns.scatterplot(data=df, x="year", y="NO3+NO2(µM)", hue="İstasyon",style="İstasyon",ax=axes[0])
...
sns.lineplot(data=df, x="year", y="S(psu)", hue="İstasyon",lw=0.65,legend=False,ax=axes[1])
sns.scatterplot(data=df, x="year", y="S(psu)", hue="İstasyon",style="İstasyon",legend=False,ax=axes[1],s=5) # s= yaparak scatterların boyutunu belirle
...
max_value=df["S(psu)"].max() #no3+no2 sütunundaki max değeri aldık, aşağıda kullanacağız
