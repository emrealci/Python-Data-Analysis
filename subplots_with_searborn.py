...
df = df.loc[( df["prSM [db]"] <=1)] # df=df [df["prSM [db]"] <= 1] ile aynı şey. (db si 1 ve 1'den kücük olan bütün sütunlar alır)
...
sns.lineplot(data=df, x="yyyy-mm-dd hh:mm:ss.sss", y="S(psu)", hue="Station",lw=0.65,legend=False,ax=axes[1])
sns.scatterplot(data=df, x="yyyy-mm-dd hh:mm:ss.sss", y="S(psu)", hue="Station",style="Station",legend=False,ax=axes[1],s=5) # s= yaparak scatterların boyutunu belirle
